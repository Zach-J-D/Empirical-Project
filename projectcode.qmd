...
vecm_df = ols_regression_df.copy()
stationary_df = vecm_df.diff().dropna()
...
# Running a cointegration test to make sure VECM is appropriate

coint_rank_test = select_coint_rank(vecm_df, 0, 4)

# Prints out a table with p-values and t-stats

coint_rank_test.summary()
...
# In case I want to do forecasting and 
...
# Specifying the models parameters and fitting the model, and printing out the results

vecm_model = VECM(stationary_df, deterministic = 'co', k_ar_diff = 3, coint_rank = 2)
...
# Taking the residuals from the wage equation as its the one of interest

wage_residuals = vecm_results.resid[:, 0]

# Plot ACF of residuals
plot_acf(wage_residuals)
plt.title('Autocorrelation of Wage Growth Residuals')
plt.show()

# Running a Shapiro-Wilk test for normality in the residuals
# Jarque-Bera test required exog and I have none since VECM takes all variables as endog

stat, p_value = stats.shapiro(wage_residuals)
...
# Setting max lags to be used

max_lag = 4

# Running a loop through every column 
# Now can individually examine the causal relationships

for col in stationary_df.columns:
    if col != 'Wage Growth':
        
        print(f'\nTesting if {col} Granger-causes Wage Growth:')
        
        grangercausalitytests(stationary_df[['Wage Growth(%)', col]], maxlag = max_lag)
...
# Setting number of years going to examine for the shock

num_periods = 12

ir = vecm_results.irf(periods = num_periods)

# Only want the first row of IRFs since that's the wage growth row

first_row_irfs = ir.irfs[:, 0, :] 

# Setting up the plot and the names

fig, ax = plt.subplots(figsize = (12, 8))

var_names = ['Wage Growth', 'Bank Rate', 'Inflation', 'Unemployment', 'OECD Economic Growth', 'Gvt Spending Growth' ]

# Loops through columns and their shocks to wage growth

for i in range(first_row_irfs.shape[1]):
    
    ax.plot(first_row_irfs[:, i], label = f'Shock to variable {var_names[i]}')


# Styling the graph with a title, a legend etc

ax.set_title('How Wage Growth Responds to Shocks')
...
# ARIMA - Testing Predictive Power of the Model
...
mean_error = np.sqrt(mean_squared_error(arimax_test['Wage Growth(%)'], forecasts_on_test))
