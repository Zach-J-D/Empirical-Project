...
stationary_df = ols_regression_df.copy()
stationary_df = stationary_df.diff().dropna()

def adf_test(df):
    adf_results = {}
    for column in stationary_df.columns:
        result = adfuller(stationary_df[column].dropna(), maxlag = 4)
        adf_results[column] = {
...
johansen_test = coint_johansen(stationary_df, det_order, lag_length)
